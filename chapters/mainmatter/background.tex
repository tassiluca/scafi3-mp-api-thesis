...
%
Ecosystem maturity is categorized as: \textit{Emerging} (limited libraries and adoption), \textit{Growing} (expanding packages ad active development), \textit{Mature} (rich ecosystem with large community) and \textit{Established} (extensive packages and widely used in industry).
%
Toolchain maturity follows a similar progression: \textit{Experimental} (minimal tooling, unstable), \textit{Developing} (basic tools available with growing tool support, but still with some instability), \textit{Mature} (production-ready tools with good stability) and \textit{Production-Grade} (robust, feature-rich tools widely adopted in production environments).
...
\begin{table}[htbp]
    ...
    $^{\leadsto}$ Indirectly through a "companion" language
\end{table}
