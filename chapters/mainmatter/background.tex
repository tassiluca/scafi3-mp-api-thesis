...
%
Over the years Scala.js has matured into a stable project allowing to target both web browsers and backends JavaScript environment, like Node.js, making it suitable for developing full-stack applications entirely in Scala.

Compilation to JavaScript is achieved via a pipeline of stages, here summarized, performed after the Scala frontend compiler have parsed and type-checked the Scala source code:
...
\begin{itemize}
    ...
    \item finally, the optimized IR is compiled into a single \texttt{.[m]js} file containing the JavaScript code ready to be executed in any JavaScript environment, such as web browsers or Node.js.
\end{itemize}
...
%
Thanks to that, Scala.js specific code can invoke JavaScript functions, manipulate DOM elements and interact with any library available in the JavaScript ecosystem through typed facades mapping JavaScript APIs to Scala.js types and abstractions.
