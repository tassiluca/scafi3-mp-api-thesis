...
This chapter aims to provide the reader the theoretical background behind the concepts of software portability, multi-platforms software development and interoperability between programming languages, unfolding how these concepts play a crucial role in the comprehensive software lifecycle and why they are relevant in modern software engineering.
...
\begin{itemize}
    ...
    \item \textbf{binary portability} involves porting software in its compiled binary format. This is the most advantageous form of portability, though it is also the most difficult to achieve and is limited to specific cases;
    \item \textbf{intermediate-level portability} is a middle-ground between source and binary portability and entails porting an intermediate representation of the software, sitting between the source and binary code. This is the case of modern languages with multi-target compilation, as discussed in \Cref{sec:approaches-to-multi-platform-and-polyglotism}.
\end{itemize}

Portability has always been a relevant concern in software engineering since the early days of computing when the landscape of hardware architectures and operating systems was extremely fragmented and heterogeneous and software was tightly coupled to the underlying platform, requiring full rewrites when moving to a different one \todo{cite?}.
%
...
%
From a programming language perspective, one influential shift addressing portability has been the move from compilation-based approaches to virtual machine-based ones, such as the Java Virtual Machine (JVM) and the Common Language Runtime (CLR) for .NET \todo{cite}.
%
These emerged to overcome the limitations of traditional compilation-based approaches, like C and C++, that require developers to create and maintain separate binaries, and sometimes even separate source code versions, for each target platform $-$ a costly and time-consuming process.
%
Virtual machine approaches addressed these challenges through an additional abstraction layer, decoupling the software from the underlying platform by compiling programs into an intermediate representation, like the Java bytecode, that is then executed by a platform-specific runtime.
%
...
%
However, the technical reality proved to be more nuanced: different virtual machine implementations can exhibit subtle differences in behavior and also system libraries can vary significantly across platforms, requiring additional adaptation efforts \todo{cite}.
%
...
%
For instance, the Node JS platform, widely used for server-side and web applications, is based on a single-threaded event loop model where blocking operations cannot be performed, requiring a completely different paradigm style based on Continuation-Passing Style (CPS) and asynchronous programming.
...
%
When designing a new software product the information about the targeted platforms is an essential input for the architect and the designer as they need to consider the constraints and capabilities of the different environments in which the software will integrate with the primary goal of reducing the \textbf{abstraction gap}, i.e., the space among the domain problem and the abstractions and capabilities offered by a platform \todo{cite}.
%
...
%
During implementation, mature ecosystems allow developers to leverage well-tested components enabling them to focus on the core business logic rather than re-implementing them from scratch, with all the drawbacks in terms of development time, costs and potential bugs that this entails.
...
%
As Doeraene notes \cite{doeraene-2018}, portability is not enough: even if a language is portable, unless the ecosystem publishes portable libraries, it remains difficult to develop applications spanning multiple runtimes for their high costs.
...
%
For example, JVM-based artifacts are distributed as JAR via Maven central or similar repositories, JavaScript applications are often bundled using tools like Webpack and distributed via npm, while Python packages are typically distributed as wheels via PyPI \todo{links?}.
...
Interoperability, however, extends beyond accessing platform-specific APIs: it entails the responsibility for library developers to expose the library APIs to the platform native language.
%
...
%
Sometimes, these differences cannot be fully bridged: some features can be dropped or limited if they cannot be mapped to the target language and are not essential for the library's core functionalities, but this must be carefully evaluated on a case-by-case basis.
%
...
%
One very famous example is the Simplified Wrapper and Interface Generator (SWIG) \todo{cite}, which can generate language bindings from C and C++ libraries' headers files to a variety of high-level languages, including Python, Java, JavaScript, Ruby and others.
%
...
%
Despite other languages, Scala cross-platform capabilities are not tied to the language itself, but rather are demanded by dedicated projects, each with its own compiler plugins and toolchains that extend the Scala compiler and ecosystem.
...
%
Over the years Scala.js has matured into a stable project allowing to target both web browsers and backends JavaScript environment, like Node.js, making it suitable for developing full-stack applications entirely in Scala, and, more recently, an experimental support for WebAssembly has been introduced \todo{cite}.

Compilation to JavaScript is achieved via a pipeline of stages, shown in \Cref{fig:scala-js-compiler-pipeline}, performed after the Scala frontend compiler have parsed and type-checked the Scala source code:

\begin{itemize}
    \item The first stage the Scala.js compiler turns Scala AST into an Intermediate Representation (IR) specifically designed to provide both portability and interoperability;
    \item then, IR files are linked together, type-checked and optimized;
    \item finally, the optimized IR is compiled into a single \texttt{.[m]js} file containing the JavaScript code ready to be executed in any JavaScript environment.
\end{itemize}
...
As for interoperability, Scala.js seamlessly integrates with existing JavaScript libraries and frameworks, thanks to a dedicated interoperability layer that allows to both consume JavaScript APIs and expose Scala.js libraries to JavaScript code.
%
Scala.js specific code can, therefore, invoke JavaScript functions, manipulate DOM elements and interact with any library available in the JavaScript ecosystem, including Node.js standard library, through typed facades mapping JavaScript APIs to Scala.js types and abstractions.
%
...
%
Unfortunately, the project doesn't support automatic generation of TypeScript type definitions from Scala.js APIs (and it is not intended to be supported in the future \todo{piè di pagina}), which would greatly ease the consumption of Scala.js libraries from TypeScript code.
%
Moreover, to allow reusing as much code as possible across the JVM and JavaScript platforms, both a subset of the Scala and Java standard libraries have been re-implemented to work natively in the JavaScript environment, providing (almost) equivalent semantics of their JVM counterparts: some difference in semantics are mostly due to the inherent difference between the two platforms (e.g., the fact only \texttt{Double}s are supported as numeric types in JavaScript) \cite{Doeraene-2025}.
%
...
%
Nevertheless, not all the JVM standard library can be ported, like all blocking I/O operations, which are not compatible with the asynchronous, non-blocking I/O model of JavaScript.
%
When designing shared-cross platform library, therefore, it is essential to carefully evaluate which parts of the code can be shared and which need to be platform-specific and guarantee that shared code relies only on cross-platform abstractions; for example, no blocking calls can be performed. \todo{move after?}

Lastly, concerning performances, Scala.js is, currently, from 2.5 to 10 times slower than equivalent hand-written JavaScript version \cite{Doeraene-2025}.
%
...
%
Compilation proceeds, similarly to Scala.js, through a pipeline of stages, shown in \Cref{fig:scala-native-compiler-pipeline}, performed after the Scala frontend compiler producing IR from which the final machine code is generated \todo{cite here or in image}:
...
\begin{itemize}
    ...
    \item foreign function and a subset of C interoperability primitives to interface with C or C++ existing libraries and native code and allowing to expose Scala Native libraries to native interoperable languages;
    \item most of the Scala and Java standard libraries re-implemented to work natively. In this way the developer can leverage familiar APIs and use existing Scala and Java standard libraries like it was running on the JVM with the same semantics (Scala Native consider, indeed, every difference in semantics between the two platforms as a bug).
\end{itemize}

Concerning the supported architectures and operating systems, Scala Native can target AMD64 (\texttt{x86-64}) and ARM64 (\texttt{aarch64}) architectures on Linux, macOS and Windows operating systems, also thanks to cross-compilation support \todo{what is it?}.
%
...
\todo{The role of C as an interoperability lingua franca.}
