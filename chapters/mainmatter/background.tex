...
\begin{itemize}
    \item in the first stage the Scala.js compiler turns scala AST into an Intermediate Representation (IR) specifically designed to provide both portability and interoperability;
...
The fact this compilation pipeline is attached to the standard Scala compiler allows to support whole Scala language, including all advanced features and abstractions.
...
%
Unfortunately, the project doesn't support (and it is not intended to be supported in the future \todo{piè di pagina}) automatic generation of TypeScript type definitions from Scala.js APIs, which would greatly ease the consumption of Scala.js libraries from TypeScript code.
%
Moreover, to allow to reuse as much code as possible across the JVM and JavaScript platforms, both a subset of the Scala and Java standard libraries have been re-implemented to work natively in the JavaScript environment, providing (almost) equivalent semantics of their JVM counterparts: some difference in semantics are mostly due to the inherent difference between the two platforms (e.g., the fact only \texttt{Double}s are supported as numeric types in JavaScript) \cite{Doeraene-2025}.
%
...
%
When designing shared-cross platform library, therefore, it is essential to carefully evaluate which parts of the code can be shared and which need to be platform-specific and guarantee that shared code relies only on cross-platform abstractions; for example, no blocking calls can be performed.
