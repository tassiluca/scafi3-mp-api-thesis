...
%
In this context, the term \textit{platform} primarily denotes the combination of hardware architecture and operating system.
%
However, very often, the term is extended to encompass also the execution runtime.
%
\textit{Environment} represents instead the broader collection of external elements with which the software interacts, including system interfaces, libraries and toolchains.
%
...
%
Portability is, therefore, a matter of engineering a software product to maximize the reusability of their components, guaranteeing the same behavior and functionalities across the different targeted platforms.
...
%
As Doeraene notes in \cite{doeraene-2018}, portability is not enough: even if a language is portable, unless the ecosystem publishes portable libraries, it remains difficult to develop applications spanning multiple runtimes due to the high development costs.
...
Scala.js is the project, introduced in 2013 at EPFL, meant to compile Scala code to JavaScript with the goal of enabling developers to write rich web applications entirely in Scala and having them compiled to ready to run JavaScript code \cite{doeraene-2018}.
%
...
Gleam is an emergent general-purpose, statically-typed functional programming language.
%
...
Differently from Scala and Kotlin, Gleam targets these platforms employing a source-to-source transpilation approach, whereby it transpiles its source code into human-readable and pretty-printed Erlang or JavaScript code with TypeScript declaration files.
%
...
%
Whereas when targeting Erlang concurrency is managed transparently by the runtime, with JavaScript Gleam requires explicit handling via promises or callbacks to deal with the event-loop model.
%
Consequently, when adopting Erlang style it is not possible to target JavaScript, and vice versa.
%
...
This positions Gleam as a language suitable for full-stack development, allowing to write both backend and frontend code in the same language, rather than as a language offering complete cross-platform capabilities.

\subsubsection{Haxe}

Haxe is general-purpose cross-platform language, primarily designed to target a large variety of platforms and programming languages, including JavaScript, PHP, Python, Lua, C++, Java and C\#, on a wide range of architectures and operating systems, like Windows, Linux, macOS, Android and Apple iOS.
%
...
%
Platform-specific code can be written using conditional compilation directives, similarly to C-style preprocessor directives.
...
Although not as popular as other languages, Haxe has found a niche in video game development and, thanks to its broad platform coverage, is a viable option for projects that require maximum platform coverage without advanced language abstractions or dependencies on specific ecosystems.

\subsubsection{Flutter/Dart}

The last analyzed cross-platform language is Dart.
%
Built by Google, Dart is a statically-typed, object-oriented language, best known for powering the Flutter framework, used to develop cross-platform applications for both mobile, native and web platforms from a single codebase.
%
It targets native platforms via Ahead-of-Time (AOT) compilation and web platforms through transpilation to JavaScript and WebAssembly.
...
\Cref{tab:comprehensive-comparison} presents a comparison of the analyzed cross-platform languages and their main characteristics.
...
\begin{table}[htbp]
    ...
    \caption{Cross-Platform Language Comparison}
    ...
    \makebox[\textwidth][c]{%
        \begin{tabularx}{1.1\textwidth}{
            ...
            >{\raggedright\arraybackslash}p{1.9cm}  % Paradigm(s)
            >{\raggedright\arraybackslash}p{1.8cm}  % Supported Platforms
            >{\raggedright\arraybackslash}p{1.8cm}  % Approach
            >{\raggedright\arraybackslash}p{1.8cm}  % Language Interoperability
            >{\raggedright\arraybackslash}p{2.0cm}  % Supported Architectures
            >{\raggedright\arraybackslash}p{2.0cm}  % Maturity
