...
\begin{itemize}
    \item in the first stage the Scala.js compiler turns Scala Abstract Syntax Tree (AST) into an IR (\texttt{.sjsir}) specifically designed to provide both portability and interoperability;
...
\begin{itemize}
    \item the first stage of the pipeline is performed by the \texttt{nscplugin} that, inspecting the AST, translates the Scala code into a strongly typed Native IR (NIR) consisting of a subset of LLVM IR instructions enriched with additional information to support Scala high level abstractions;
...
\subsection{Comparative Analysis of Language Ecosystems}
...
\begin{table}[htbp]
    ...
    \makebox[\textwidth][c]{%
        \begin{tabularx}{\textwidth}{
            ...
            >{\raggedright\arraybackslash}p{1.3cm}  % Maturity
...
%\todo{Erlang and Elixir for Gleam?}
%\todo{Scala - Android platform?}
