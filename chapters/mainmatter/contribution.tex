...
%
This is achieved through well-defined Platform Interfaces that abstract platform-specific capabilities and services. 
%
These interfaces are designed to balance the library's functional requirements with the paradigms and constraints of each target platform, such as using asynchronous APIs that can be consistently mapped across all supported platforms.
%
...
%
This tension is particularly relevant given that different programming languages offer distinct type systems and abstractions that cannot always be straightforwardly mapped.
...
%
Its main focus is to maximize code sharing and reuse across all supported platforms and programming languages, while minimizing platform-specific code and complexity, and provide a consistent and unified API to library users.
...
\begin{enumerate}
    ...
    \item \textbf{Performance Overhead.} Interoperability across programming languages is achieved through type conversion, which introduces performance overhead, particularly when handling complex data structures or large data volumes. However, this overhead remains significantly lower than alternative approaches based on remote procedure calls (RPC) or inter-process communication (IPC) primitives, which incur additional serialization and network/process boundary crossing costs.
    \item \textbf{Data Expressiveness.} The expressiveness of exchanged messages between nodes is limited to the capabilities of the adopted cross-platform and polyglot serialization format. While this normally does not pose significant limitations, since exchanged messages are represented as \textit{records}--—immutable data classes composed of fields with no additional behavior--—it is necessary to consider that complex data structures or language-specific constructs may not be fully representable, potentially requiring simplifications or alternative representations.
\end{enumerate}
