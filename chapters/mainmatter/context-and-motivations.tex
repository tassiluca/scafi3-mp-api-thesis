...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{./resources/img/gradient-round0.pdf}
    \caption{A visual representation of the gradient field computed by the program in \Cref{lst:scafi3-gradient-example} at round 1. The source node is represented in yellow. Links are labeled with the sensed distance values, while nodes are labeled with the computed gradient value.}
    \label{fig:gradient-round0}
\end{figure}
