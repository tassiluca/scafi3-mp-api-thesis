...
%
When evaluating the neighboring values, however, only the values produced by devices evaluating the same \texttt{nbr} in the same branch should be considered, otherwise the program would mix values coming from different contexts, resulting in incorrect outcomes.
%
...
%
In \Cref{fig:alignment-example}, for instance, when evaluating the \texttt{nbr} expression, each device can only observe neighboring values coming from devices with the same parity of identifier.
...
Over the years, several other implementations have been developed---or are currently being developed---in different languages, including FCPP \cite{fcpp} in C++, Collektive \cite{collektive}\cite{collektive2} in Kotlin, Rufi \cite{rufi} in Rust.
%
...
More generally, abstracting away from AC, the goal of this work is to investigate the feasibility of building a multi-platform and multi-language architecture for building Scala distributed, cross-platform and polyglot libraries.
%
Ultimately, such architecture could be reified into the AC domain.
%
More specifically, this exploration is meant to investigate:
%
\begin{itemize}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}{0.40\textwidth}
    \centering
    \includegraphics[width=\textwidth]{./resources/img/gradient-round0.pdf}
  \end{minipage}
