...
%
The rationale behind these multiple implementations is to reach the widest possible range of platforms, enabling real-world deployments on edge devices, and to leverage the unique strengths of each programming language and its ecosystem, such as performance optimizations in C++ or Python Machine and Deep Learning libraries.
%
However, these implementations have been created independently of scratch, with no code reuse and not compatibility in mind.
%
Moreover, the lack of a common framework has led to fragmentation in the ecosystem, leading to a situation where each implementation has its own set of libraries and ad hoc extensions.
